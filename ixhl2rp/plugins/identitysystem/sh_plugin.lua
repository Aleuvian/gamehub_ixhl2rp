...
if CLIENT then
	...
	netstream.Hook("SubmitNewCID", function(ply, data)
		if ply:IsCombine() then
			...
			local data2 = {
				["citizen_name"] = data[1],
				["cid"] = inv:GetItemsByUniqueID("transfer_papers"):GetData("id", id)
				["issue_date"] = TimeString,
				["officer"] = ply:Name()
			}

			inv:Add("cid_new", 1, data2)
...
local BUILD_LOYALTY = {
	["metropolice_wages"] = {
		...
		["desc"] = "Civil Protection wage packet.",
		["results"] = {["tokens"] = 125},
		["cooldown"] = 600,
		["data"] = {
			["disables"] = {"tokens"},
	...
	["metropolice_ration"] = {
		...
		["desc"] = "A highly nutritious, calorie dense ration packet.",
